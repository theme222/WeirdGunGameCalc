...
const pages = [
  ...
  `To Start, select a filter type from the dropdown menu and press "Add Filter". There will be 3 required filters which can't be removed: Categories (The allowed categories for the core), Sort Type (The order of the output list), and Total Results (How many to show).`,
  ...
  `Other than bruteforcing combinations, this website can also be used to simply compute the result of 1 combination. By forcing the barrel, magazine, grip, stock and core, you no longer need to wonder what the stats of the gun will be in game! Another usecase is to ungatekeep guns. If you ever see a rogue stat card that someone is gatekeeping the parts of, you can simply filter by range each stat (± 0.1 for errors) to find the exact combination they used!`,
  `This website uses Web Assembly to run c++ code on a browser. So while it is pretty fast, you can get better speeds by using the CLI (Since that allows multi threading). Currently it also doesn't have the option to set default max health and banning price types like the CLI. Lastly it doesn't have a way to specify scopes (but honestly who cares about scopes anyways lol).`,
