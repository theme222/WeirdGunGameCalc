import { ref } from 'vue';
import { currentFilters, filterAndSortStrings } from './filter';
...
export const WasmCalc = new WggCalcManager();
...
export async function runCalc() {
  ...
  const totalVal = currentFilters.list.find(f => f.title === "Total Results")?.writeable.value[0] as number;
  const sortType = sortTypeJSToWasm(currentFilters.list.find(f => f.title === "Sort Type")?.writeable.value[0] as string);
  const sortPriority = sortPriorityJSToWasm(currentFilters.list.find(f => f.title === "Sort Type")?.writeable.selectedOption as string);
  const categories = currentFilters.list.find(f => f.title === "Categories")?.writeable.value as string[];
  
  if (totalVal && sortType && sortPriority && categories) {
    await WasmCalc.addFilterRequired(totalVal, sortType, sortPriority, categories);
  }
  
  for (const filter of currentFilters.list) {
